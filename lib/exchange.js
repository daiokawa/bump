...
import { seal, open, signMessage, verifyMessage, makeUnsendClaim } from './crypto.js';
import { buildMessage, messageIsSane } from './message.js';
import { logEvent, writeJson, readJson, listBox, messageExists, readPeer, writePeer, readLog } from './store.js';
import { unsendFromRelay } from './relay.js';
...
// --- 送信取り消し -----------------------------------------------------------
// 「未読のうちに取り消す」の実際の境界は既読/未読ではなく配達前/後。中継にまだ在る封筒は
// 消せるが、相手の端末へpullされた後は手が届かない（相手の端末には触れない＝invariant）。
// 取り消せたら sent-status を unsent にし、履歴の手紙本体（messages/）は消さず残す。
export async function unsendMessage({ identity, pairId, messageId, relayDir, peerDeviceId, now }) {
  const p = pairPaths(pairId);
  const ev = (await readLog(pairId)).find((e) => e.type === 'sent' && e.message_id === messageId);
  if (!ev || !ev.envelope_id) throw new Error('not a letter you sent (no send record)');
  const envelopeId = ev.envelope_id;
  let removed = false;
  // まだ投函前（outboxに生のまま）なら、家から出る前に止めるだけで済む。
  try { await rename(join(p.outbox, `${envelopeId}.env.json`), join(p.outbox, `${envelopeId}.env.json.unsent`)); removed = true; }
  catch {
    const claim = makeUnsendClaim(identity, envelopeId, now());
    ({ removed } = await unsendFromRelay({ pairId, relayDir, peerDeviceId, claim }));
    if (removed) await rename(join(p.outbox, `${envelopeId}.env.json.pushed`), join(p.outbox, `${envelopeId}.env.json.unsent`)).catch(() => {});
  }
  if (removed) {
    const st = await readSentStatus(pairId);
    st[messageId] = { status: 'unsent' };
    await writeJson(statusPath(pairId), st);
    await logEvent(pairId, { type: 'unsent', message_id: messageId, envelope_id: envelopeId }, now());
  }
  return { removed, envelope_id: envelopeId };
}
