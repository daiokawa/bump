...
import { mkdir, copyFile, rename, readdir, unlink, readFile, writeFile, stat } from 'node:fs/promises';
...
// 送信取り消し：中継の受信箱にまだ在る封筒を、差出人の署名付き要求で取り除く。
// 取り消せるのは配達前（相手がpullする前）だけ。無ければ removed:false（冪等）。
export async function unsendFromRelay({ pairId, relayDir, peerDeviceId, claim }) {
  const url = relayUrl();
  if (url) {
    const r = await fetch(url.replace(/\/$/, '') + '/unsend', { method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ to_routing_id: peerDeviceId, claim }) });
    if (!r.ok) throw new Error(`relay unsend ${r.status}`);
    return { removed: !!(await r.json()).removed };
  }
  const f = join(mailbox(relayDir, pairId, peerDeviceId), `${claim.envelope_id}.env.json`);
  try { await unlink(f); return { removed: true }; } catch { return { removed: false }; }
}
...
export async function pullConnectRequests({ relayDir, myDeviceId }) {
  ...
  if (url) {
    ...
    const get = async (after) => {
      const res = await fetch(`${url.replace(/\/$/, '')}/get?routing_id=${encodeURIComponent(myDeviceId)}&after=${after}`);
      if (!res.ok) throw new Error(`relay get ${res.status}`);
      return res.json();
    };
    let { items, cursor: next, max } = await get(cursor);
    // 中継の受信箱が巻き戻ったとき（TTLで空になり番号が1から振り直された等）、こちらの
    // cursorだけが先に進んだままになり、空の応答を受け取り続けて配達が黙って止まる。
    // 自分のcursorが中継の最大番号より先にいたら、それを検知して取り直す
    // （2026-08-12に実発生。1週間、手紙も受領も届かなかった＝Y.K.報告）。
    if (typeof max === 'number' && cursor > max) {
      warn('中継の受信箱が巻き戻っていました。取り直します', new Error(`cursor ${cursor} > relay max ${max}`));
      cursor = 0;
      ({ items, cursor: next, max } = await get(0));
    }
...
// この封筒は既に手元にあるか。未処理の .env.json だけでなく、取り込み済みの名残
// （.done/.dup/.rejected）も「ある」と数える。巻き戻り復旧の取り直しは既知の封筒を
// もう一度連れてくるので、置き直すと復号やり直しのムダと、pull件数が既知分まで
// 数えられる件（K.S.指摘 2026-08-20）が出る。
async function alreadyHave(inboxDir, envelopeId) {
  for (const suf of ['', '.done', '.dup', '.rejected']) {
    try { await stat(join(inboxDir, `${envelopeId}.env.json${suf}`)); return true; } catch {}
  }
  return false;
}
...
export async function pullAllFromRelay({ relayDir, myDeviceId, peersOf }) {
  const url = relayUrl();
  const placed = {};                        // pairId → 新しく置いた件数
  const put = async (pairId, env) => {
    const inboxDir = pairPaths(pairId).inbox;
    if (await alreadyHave(inboxDir, env.envelope_id)) return;
    await writeFile(join(inboxDir, `${env.envelope_id}.env.json`), JSON.stringify(env, null, 2), 'utf8');
  ...
  if (url) {
    ...
    let { items, cursor: next, max } = await get(cursor);
    // 巻き戻り検知。この関数はコンソール/画面の取り込み経路で、2026-08-19の修正
    // （pullFromRelay と pullConnectRequests）から漏れていた＝コミット文は「両経路」と
    // 言いながらここだけ素通しだった。中継の最大番号より自分が先にいたら取り直す。
    if (typeof max === 'number' && cursor > max) {
      warn('中継の受信箱が巻き戻っていました。取り直します', new Error(`cursor ${cursor} > relay max ${max}`));
      cursor = 0;
      ({ items, cursor: next, max } = await get(0));
...
export async function pullFromRelay({ pairId, relayDir, myDeviceId, peerDeviceId }) {
  ...
  if (url) {
    ...
    const get1 = async (after) => {
      const res = await fetch(`${url.replace(/\/$/, '')}/get?routing_id=${encodeURIComponent(myDeviceId)}&after=${after}`);
      if (!res.ok) throw new Error(`relay get ${res.status}`);
      return res.json();
    };
    let { items, cursor: next, max } = await get1(cursor);
    // 巻き戻り検知（pullAllFromRelay と同じ理屈）。中継の最大番号より自分が先にいたら取り直す。
    if (typeof max === 'number' && cursor > max) {
      warn('中継の受信箱が巻き戻っていました。取り直します', new Error(`cursor ${cursor} > relay max ${max}`));
      cursor = 0;
      ({ items, cursor: next, max } = await get1(0));
    }
    let n = 0;
    for (const it of items) {
      const env = it.envelope;
      ...
      if (await alreadyHave(p.inbox, env.envelope_id)) continue;        // 取り直しの既知分は数えない
