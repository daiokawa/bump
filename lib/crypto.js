...
// --- 送信取り消し（unsend）の署名付き要求 -----------------------------------
// 中継は鍵台帳を持たない（目隠し）。それでも「差出人本人の要求」だけを通せるのは、
// device_id = 公開鍵のハッシュ だから。要求に公開鍵を同梱し、中継は
// (1) その鍵から導いたdevice_idが封筒のfromと一致 (2) 署名が正当 (3) 時刻が新しい
// の3点だけ確かめる。誰なのかは知らないまま、封筒の差出人本人であることだけ分かる。
export function makeUnsendClaim(identity, envelopeId, nowIso) {
  const body = { v: 1, kind: 'mm-unsend', envelope_id: envelopeId,
    device_id: identity.device_id, ed25519_pub: identity.ed25519.pub, at: nowIso };
  const sig = edSign(null, canonicalBytes(body), importPriv(identity.ed25519.priv));
  return { ...body, sig: b64(sig) };
}
export function verifyUnsendClaim(claim, nowIso, skewMs = 10 * 60 * 1000) {
  if (!claim || claim.v !== 1 || claim.kind !== 'mm-unsend' || !claim.sig
    || typeof claim.envelope_id !== 'string' || typeof claim.ed25519_pub !== 'string')
    return { ok: false, reason: 'malformed' };
  if (deviceIdFromEdPub(claim.ed25519_pub) !== claim.device_id) return { ok: false, reason: 'key/device mismatch' };
  const { sig, ...body } = claim;
  let good = false;
  try { good = edVerify(null, canonicalBytes(body), importPub(claim.ed25519_pub), unb64(sig)); } catch {}
  if (!good) return { ok: false, reason: 'bad signature' };
  if (Math.abs(Date.parse(nowIso) - Date.parse(claim.at)) > skewMs) return { ok: false, reason: 'stale claim' };
  return { ok: true, device_id: claim.device_id, envelope_id: claim.envelope_id };
}
