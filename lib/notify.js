...
export async function osaNotify(text, title = 'middleman') {
  if (process.platform !== 'darwin') return;
  await execFileP('osascript', ['-e',
    `display notification ${JSON.stringify(text)} with title ${JSON.stringify(title)}`]);
}

// Chatworkへの新着通知（北原さん方式の公式化）。渡すのは「誰から何件」だけ＝本文は載せない。
// トークンは各自の ~/.middleman/notify.json にのみ保存（プロフィール伝播には乗せない）。
// self_unread=1: 自分の投稿でも未読バッジを立てる（自分の部屋へ自分で投げる運用のため。小坂井Claude検証 2026-07-22）。
